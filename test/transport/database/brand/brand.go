...
//		&result.UUID, &result.LocalizedNames, &result.Alias, &result.Description, &result.NodeUUID,
//		&result.Status, &result.Sort, &result.CreatedAt, &result.CreatedBy,
//		&result.UpdatedAt, &result.UpdatedBy, &result.Icon, &result.Logo,
//	); err != nil {
...
// CleanBrand представляет собой структуру для сериализации
...
// Brand - структура, представляющая данные из базы для преобразования
type Brand struct {
	UUID           string          `db:"uuid"`
	LocalizedNames json.RawMessage `db:"localized_names"`
	Alias          string          `db:"alias"`
...
func (r *Repository) GetBrand(ctx context.Context, brandUUID uuid.UUID) (*CleanBrand, error) {
	...
	// Преобразуем результат в CleanBrand
	cleanBrand := &CleanBrand{
		UUID:           result.UUID,
		LocalizedNames: result.LocalizedNames,
		Alias:          result.Alias,
		Description:    result.Description.String, // Получаем значение, если оно Valid
	...
	return cleanBrand, nil // Возвращаем CleanBrand
}
