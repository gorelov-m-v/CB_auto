...
// 		confirmationContext, _ := testData.emailConfirmationMessage.GetConfirmationContext()
...
// 		confirmationContext, _ := testData.phoneConfirmationMessage.GetConfirmationContext()
...
func (s *SingleBetLimitSuite) TestSingleBetLimit(t provider.T) {
	...
	t.WithNewStep("Создание и верификация игрока", func(sCtx provider.StepCtx) {
		testData.authorizationResponse = defaultSteps.CreateVerifiedPlayer(
			sCtx,
			s.publicClient,
			s.capClient,
			s.kafka,
