...
func (s *SingleBetLimitSuite) TestSingleBetLimit(t provider.T) {
	...
	t.WithNewAsyncStep("Получение лимита на одиночную ставку", func(sCtx provider.StepCtx) {
	...
	t.WithNewAsyncStep("Проверка сообщения в NATS о создании лимита на одиночную ставку", func(sCtx provider.StepCtx) {
		...
		eventType := singleBetEvent.Payload.EventType
		...
		sCtx.Assert().Equal(eventType, singleBetEvent.Payload.EventType, "Тип события совпадает")
	})
}
